    ...
    if (previewItems.length === 0) {
        ...
        previewItems.forEach(item => {
            ...
            const mediaMarkup = imgUrl
                ? `<img src="${escapeHtml(imgUrl)}" class="past-prize-thumb img-loading" alt="Past Prize" onload="this.classList.remove('img-loading')">`
...
function buildSeeMoreGrid() {
    ...
    activePastPrizesData.forEach((item, index) => {
        ...
        const imgUrl = item.imageUrl || item.url || "";
        const mediaMarkup = imgUrl
            ? `<img src="${escapeHtml(imgUrl)}" class="see-more-img img-loading" alt="Past Prize" onload="this.classList.remove('img-loading')" onclick="openImagePreview('${escapeHtml(imgUrl)}')">`
...
function buildNoticeCarousel(items) {
    ...
    if (activeNoticeItems.length === 0) {
        ...
        activeNoticeItems.forEach(item => {
            ...
            const mediaMarkup = imgUrl
                ? `<img src="${escapeHtml(imgUrl)}" alt="Notice" class="prize-image img-loading" loading="eager" fetchpriority="high" decoding="async" onload="this.classList.remove('img-loading')">`
                : `<div class="prize-fallback-placeholder">${IMAGE_PLACEHOLDER_SVG_MARKUP}</div>`;
