...
(function () {
    ...
    // ---- Scroll lock (initial load + network-error overlay states) ----------------
    // Keys that would otherwise move the page (Space/Arrows/Page/Home/End) are only
    // swallowed when focus isn't inside a text field, so nothing else on the page is
    // affected once the overlay is gone.
    ...
    function unlockScroll() {
        ...
        document.removeEventListener("touchmove", blockScrollGesture, { passive: false });
        document.removeEventListener("wheel", blockScrollGesture, { passive: false });
        document.removeEventListener("keydown", blockScrollKey, { passive: false });
    }

    // The preloader is already visible by default at first paint (before any JS runs),
    // so the lock must engage immediately rather than waiting for the first showPreloader() call.
    lockScroll();
    ...
    window.TZeroneLoader = { show: showPreloader, hide: hidePreloader, lockScroll: lockScroll, unlockScroll: unlockScroll };
})();

/* =====================================================================================
   IMAGE DOWNLOAD / LONG-PRESS GUARD
   Blocks the native "download / open image / select" popup on every image site-wide
   (logo, mascots, hero, dashboard, game section, etc). Delegated on document so it
   covers images rendered later by Firestore listeners (past prizes, winners, etc) too.
   The iOS long-press callout is handled separately in CSS (-webkit-touch-callout).
   The only place an image can still be downloaded is via the explicit DOWNLOAD IMAGE
   button inside the Image Preview modal (see openImagePreview below) — untouched here.
===================================================================================== */
...
/* =====================================================================================
   GLOBAL BODY SCROLL LOCK (popup / overlay background freeze)
   Shared by every popup — Winner popup, Image Preview, the Country picker, the custom
   Alert/Confirm dialog, and the "See More" Past Prizes overlay. Reference-counted so
   stacked overlays (e.g. tapping a prize image inside the Winner popup opens Image
   Preview on top of it) never unlock the background while another popup is still open
   underneath — only the matching outermost close call actually restores scrolling.
   Pins the body with position:fixed at the exact scroll offset instead of relying on
   plain overflow:hidden: overflow:hidden alone does not reliably stop touch-driven
   background scrolling on iOS Safari, and lets the elastic rubber-band overscroll bounce
   past the document's real bounds — which is what exposed the unfilled/cut-out gaps at
   the bottom of the background whenever a popup was open. Freezing the body as a
   fixed, full-height box removes any document scroll for the browser to bounce against,
   so the page background stays completely fixed and seamlessly filled underneath.
===================================================================================== */
let bodyScrollLockCount = 0;
let bodyScrollLockOffset = 0;

function lockBodyScroll() {
    bodyScrollLockCount++;
    if (bodyScrollLockCount > 1) return; // already locked by another open popup
    bodyScrollLockOffset = window.scrollY || window.pageYOffset || 0;
    document.body.style.top = -bodyScrollLockOffset + "px";
...
function unlockBodyScroll() {
    ...
    bodyScrollLockCount--;
    if (bodyScrollLockCount > 0) return; // another popup is still open — stay locked
    document.body.classList.remove("modal-scroll-lock");
    document.body.style.top = "";
    window.scrollTo({ top: bodyScrollLockOffset, left: 0, behavior: "instant" });
}

/* =====================================================================================
   GLOBAL STATE & CONFIG CONSTANTS
   In-memory app state, asset URLs, and static reference data (country list, placeholder
      markup) shared across every feature module below.
===================================================================================== */
...
// The Game tab's featured "CLASH BLADE" banner previously only started downloading the
// moment the Game tab became active, which is exactly when the user is looking straight
// at the empty frame waiting for it. Same background-cache technique as the mascot
// preload above: fire a plain Image() request for the same URL the real
// <img id="game-featured-image"> in the Game tab uses, while the user is still browsing
// Home/Dashboard. The browser's HTTP image cache then serves that exact URL instantly
// (no network round-trip, no layout shift, no blank placeholder) the moment the Game
// tab's <img> is actually painted.
const GAME_FEATURED_IMAGE = "https://raw.githubusercontent.com/TARHIT-T0SVX/All-image-t-z/refs/heads/main/Game/clash_blade_official_icon_T01.jpeg";
function preloadGameAssets() {
    const img = new Image();
    img.src = GAME_FEATURED_IMAGE;
}
...
const IMAGE_PLACEHOLDER_SVG_MARKUP = `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g stroke-width="0"/><g stroke-linecap="round" stroke-linejoin="round"/><g><path fill-rule="evenodd" clip-rule="evenodd" d="M6.205 3h11.59c1.114 0 1.519.116 1.926.334.407.218.727.538.945.945.218.407.334.811.334 1.926v7.51l-4.391-4.053a1.5 1.5 0 0 0-2.265.27l-3.13 4.695-2.303-1.48a1.5 1.5 0 0 0-1.96.298L3.005 18.15A12.98 12.98 0 0 1 3 17.795V6.205c0-1.115.116-1.519.334-1.926.218-.407.538-.727.945-.945C4.686 3.116 5.09 3 6.205 3zm9.477 8.53L21 16.437v1.357c0 1.114-.116 1.519-.334 1.926a2.272 2.272 0 0 1-.945.945c-.407.218-.811.334-1.926.334H6.205c-1.115 0-1.519-.116-1.926-.334a2.305 2.305 0 0 1-.485-.345L8.2 15.067l2.346 1.508a1.5 1.5 0 0 0 2.059-.43l3.077-4.616zM7.988 6C6.878 6 6 6.832 6 7.988 6 9.145 6.879 10 7.988 10 9.121 10 10 9.145 10 7.988 10 6.832 9.121 6 7.988 6z" fill="#ffffff"/></g></svg>`;

/* =====================================================================================
   IMAGE LOAD RETRY / BACKOFF
   A prize/winner image URL can transiently fail to load (a flaky mobile connection, a
   momentary CDN hiccup) even though the URL itself is perfectly valid. Previously any
   load failure fell straight through to the fallback icon with no second chance. Every
   prize/winner <img> now gets exactly one automatic retry after a short delay before
   giving up — enough to ride out a transient blip without masking a genuinely broken
   URL behind an infinite retry loop.
===================================================================================== */
const IMAGE_RETRY_DELAY_MS = 1500;

/**
 * Attach a one-time retry-then-fallback handler to an <img> element.
 * @param {HTMLImageElement} imgEl - the image element to watch.
 * @param {string} url - the original (clean) URL, kept separately so the retry
 *   attempt isn't built on top of an already cache-busted src.
 * @param {Function} onFinalError - called only if the retry attempt also fails.
 */
function attachImageRetryHandler(imgEl, url, onFinalError) {
    if (!imgEl || !url) return;
    imgEl.dataset.tzOrigSrc = url;
    imgEl.dataset.tzRetried = "0";
    imgEl.onerror = () => {
        if (imgEl.dataset.tzRetried === "0") {
            imgEl.dataset.tzRetried = "1";
            setTimeout(() => {
                // Cache-bust so the browser re-attempts the network request instead of
                // instantly replaying the same failed response from its error cache.
                const baseUrl = imgEl.dataset.tzOrigSrc || url;
                const bustUrl = baseUrl + (baseUrl.includes("?") ? "&" : "?") + "_tzretry=" + Date.now();
                imgEl.src = bustUrl;
...
/**
 * Same retry-then-fallback behavior as attachImageRetryHandler, but for images rendered
 * via innerHTML template strings (past prize thumbs, see-more grid, notice carousel)
 * where there's no direct element reference at render time. Wired up via an inline
 * onerror="handleTzImageError(this)" attribute; on final failure it swaps itself for
 * the same placeholder icon markup used when no image URL exists at all.
 */
window.handleTzImageError = function(imgEl) {
    if (!imgEl) return;
    const attempts = parseInt(imgEl.dataset.tzRetried || "0", 10);
    const baseUrl = imgEl.dataset.tzOrigSrc || imgEl.src;
    if (attempts < 1) {
        imgEl.dataset.tzRetried = "1";
        setTimeout(() => {
            const bustUrl = baseUrl + (baseUrl.includes("?") ? "&" : "?") + "_tzretry=" + Date.now();
        ...
        placeholder.className = "prize-fallback-placeholder" + (imgEl.classList.contains("see-more-img") ? " see-more-img" : "");
        placeholder.innerHTML = IMAGE_PLACEHOLDER_SVG_MARKUP;
        if (imgEl.parentNode) imgEl.replaceWith(placeholder);
    }
};
...
window.customAlert = function(title, message, type = 'info') {
    return new Promise(resolve => {
        ...
        if (overlay.classList.contains("hidden")) lockBodyScroll();
...
/* =====================================================================================
   APP BOOTSTRAP / DOM-READY INIT
   Wires up all static interaction listeners once the DOM is parsed (before any
      Firestore data has arrived).
===================================================================================== */
...
/* =====================================================================================
   SERVICE WORKER — OFFLINE-FIRST ASSET SHELL
   Pairs with the existing localStorage live-data fallback: that cache lets a repeat
   visit render stale-but-correct Firestore data with no connection, but the page shell
   itself (HTML/CSS/JS/fonts/images) still had to come over the network first. Registered
   after the rest of boot so it never competes with the critical first paint. Entirely
   additive — does not touch, wrap, or intercept anything Firestore-related; see sw.js.
===================================================================================== */
function registerAppServiceWorker() {
    if (!("serviceWorker" in navigator)) return;
    window.addEventListener("load", () => {
        navigator.serviceWorker.register("sw.js").catch(err => {
            // Non-fatal: the app already works fully online without it, and the
            // localStorage fallback still covers stale-data-while-offline.
...
// - Caps the total length (including the leading "#") at 16 characters
...
/* =====================================================================================
   STATIC UI INTERACTION LISTENERS
   Buttons/inputs that don't depend on live Firestore data (nav, form, modals).
===================================================================================== */

function setupInteractionListeners() {
    // Game ID field: forces a single leading "#", restricts input to A-Z / 0-9 / spaces,
    // auto-uppercases in real time, and caps the total length (including the leading "#")
    // at 16 characters — matches sanitizeGameIdInput() and the input's maxlength="16".
...
/* =====================================================================================
   HOME CAROUSEL / SLIDER ENGINE
   Generic swipe-carousel controller reused by every home-page carousel instance.
===================================================================================== */
...
function setupSliderObserver() {
    ...
    initSwipeCarousel("game-carousel-track", "game-slider-btn-left", "game-slider-btn-right", "game-slider-wrapper");
}
...
/* =====================================================================================
   ...
   Maps the phone's physical back button (and the browser's back gesture) onto exact,
   single-step layer transitions. Every page/overlay the user opens pushes one History
   entry, so one back press always undoes exactly one layer:
     - Dashboard / Game -> Home directly (one press)
     - Fill the Details: back disabled entirely (only the "BACK TO HOME" button works)
     - Confirm Details: back acts like EDIT (returns to Fill the Details)
     - Submission Successful: back disabled entirely (only the on-screen button works)
     - Past Prizes popup + image preview: 1st press closes the preview, 2nd closes the
       popup, landing back on Home at the exact same scroll position
     - Dashboard winner popup + image preview: 1st press closes the preview, 2nd closes
       the winner popup, 3rd leaves the Dashboard for Home
   A single in-memory stack (navLayerStack) tracks one entry per History push, and one
   popstate listener undoes exactly its top entry per press.
===================================================================================== */

const TRAPPED_LAYERS = new Set(["form", "success"]);
const TOP_LEVEL_LAYERS = new Set(["home", "dashboard", "game"]);

// Primary top-level pages (the three bottom-nav tabs). Direct switches between these
// always land at the top, instantly. Returning to one of them from a sub-section/detail
// layer (Fill the Details, Confirm, Success, See More, etc.) instead restores exactly the
// scroll position the user had before they left — tracked here per page id.
const TOP_LEVEL_PAGE_IDS = new Set(["page-home", "page-dashboard", "page-game"]);
let topLevelScrollMemory = {};

const PAGE_ID_TO_LAYER = {
    "page-home": "home",
    ...
    "page-see-more": "see-more"
};

let navLayerStack = ["home"];
// Set right before any history.back() call WE make ourselves (EDIT button, close-X
// buttons, "BACK TO HOME" buttons, etc.) so the resulting popstate knows this was an
// explicit on-screen action rather than a genuine hardware/gesture back press — the
// only kind the Fill Details / Submission Successful trap is meant to swallow.
let intentionalBackInProgress = false;

// Establishes the base History entry for this session. Nothing is ever popped below it.
history.replaceState({ tzBase: true }, "");

// We own scroll restoration ourselves (see TOP_LEVEL_PAGE_IDS / topLevelScrollMemory
// above) instead of letting the browser silently re-apply its own remembered offset on
// a history navigation, which is what produced the visible auto-scroll-down bug.
if ("scrollRestoration" in history) {
    history.scrollRestoration = "manual";
}

function currentNavLayer() {
    return navLayerStack[navLayerStack.length - 1];
}

function pushNavLayer(layerId) {
    navLayerStack.push(layerId);
    history.pushState({ tzLayer: layerId }, "");
}

function replaceNavLayer(layerId) {
    navLayerStack[navLayerStack.length - 1] = layerId;
    history.replaceState({ tzLayer: layerId }, "");
}

function resetNavLayerToHome() {
    navLayerStack = ["home"];
    history.replaceState({ tzBase: true }, "");
}

// Simulates a single hardware back press programmatically — used by on-screen controls
// (EDIT, close-X buttons, See More's own close button) that are specified to behave
// exactly like a physical back press, so both paths share the one popstate handler below.
function goBackOneNavLayer() {
    intentionalBackInProgress = true;
    history.back();
}
...
window.navigateTo = function(targetPageId) {
    ...
    const layerId = PAGE_ID_TO_LAYER[targetPageId];
    if (!layerId) {
        applyPageTransition(targetPageId);
        return;
    }

    // Requesting the layer directly beneath the current top (Confirm's EDIT button
    // targeting Fill the Details, or a "BACK TO HOME"/tab button targeting Home from one
    // level up) is functionally identical to a single back press — route it through the
    // same history.back() + popstate path instead of growing the stack with a duplicate.
    const parentLayer = navLayerStack.length > 1 ? navLayerStack[navLayerStack.length - 2] : null;
    if (layerId === parentLayer) {
        goBackOneNavLayer();
        return;
    }
    ...
    if (layerId === "home") {
        resetNavLayerToHome();
    } else if (layerId === "see-more") {
        if (currentNavLayer() !== "see-more") pushNavLayer("see-more");
    } else if (TOP_LEVEL_LAYERS.has(layerId)) {
        if (navLayerStack.length <= 1) pushNavLayer(layerId);
        else replaceNavLayer(layerId);
    } else {
        pushNavLayer(layerId);
    }

    applyPageTransition(targetPageId);
};

// Applies the actual close/return effect for whichever layer a back press (or an
// on-screen control acting like one) just removed.
function applyNavLayerRemoval(removedLayer) {
    switch (removedLayer) {
        case "image-preview":
        ...
        case "winner-modal":
            ...
            break;
        case "confirm":
            applyPageTransition("page-form");
            break;
        default:
            // see-more, dashboard, game, form, and success all resolve back to Home.
            applyPageTransition("page-home");
            break;
    }
}

window.addEventListener("popstate", () => {
    const wasIntentional = intentionalBackInProgress;
    intentionalBackInProgress = false;

    const topLayer = currentNavLayer();

    // Genuine hardware/gesture back press while a trapped page is open: swallow it and
    // re-plant a fresh entry so the next press is caught the same way — a consumed entry
    // doesn't grow back on its own. On-screen controls that route through
    // goBackOneNavLayer() are exempt, since they ARE the page's sanctioned exit.
    if (!wasIntentional && TRAPPED_LAYERS.has(topLayer) && navLayerStack.length > 1) {
        history.pushState({ tzLayer: topLayer }, "");
        return;
    }

    if (navLayerStack.length <= 1) {
        // Already at the Home base — nothing above it to close/undo. Do nothing at all
        // (in particular, no scroll change) and let the press fall through natively.
        return;
    }

    navLayerStack.pop();
    applyNavLayerRemoval(topLayer);
});

/* =====================================================================================
   CORE PAGE / OVERLAY TRANSITION
   Pure DOM effect of moving to a given page id — no History/back-stack bookkeeping.
   Called by navigateTo() (which owns the stack) and by the back-button handler above
   when it needs to re-apply this same visual effect for a pop.
===================================================================================== */
function applyPageTransition(targetPageId) {
    const seeMorePage = document.getElementById("page-see-more");
    const seeMoreIsOpen = seeMorePage.classList.contains("active");

    // "See More" (Past Prizes) is a full-screen overlay layered on top of whatever real page
    // is already active underneath (normally Home) — it is NEVER allowed to hide that page or
    // touch the window's scroll position. Previously, opening it hid the Home page entirely,
    // which collapsed the document's scrollable height and forced the browser to silently
    // reset scroll to 0; closing it then had to restore that offset, which produced a visible
    // re-scroll animation. Leaving the underlying page active the whole time means there is
    // nothing to freeze and nothing to restore — the background simply never moves.
    if (targetPageId === "page-see-more") {
        if (!seeMoreIsOpen) {
            // Remember exactly where Home (or whichever top-level page is underneath) was
            // scrolled to, so closing See More can be a no-op for scroll (see below) even
            // though this whole block is left untouched from before.
            const activeBeneath = document.querySelector(".view-page.active:not(#page-see-more)");
            if (activeBeneath && TOP_LEVEL_PAGE_IDS.has(activeBeneath.id)) {
                topLevelScrollMemory[activeBeneath.id] = window.scrollY;
            }
            seeMorePage.classList.add("active");
            lockBodyScroll();
    ...
    const underlyingActiveNode = document.querySelector(".view-page.active:not(#page-see-more)");
    const underlyingActiveId = underlyingActiveNode ? underlyingActiveNode.id : null;

    if (seeMoreIsOpen) {
        seeMorePage.classList.remove("active");
        unlockBodyScroll();
        // Closing back onto the same page that was already showing underneath (the normal
        // close-button / back-button path) — that page never left, so leave scroll exactly
        // where the user had it instead of forcing a jump to the top.
        if (targetPageId === underlyingActiveId) {
            updateBottomNavState(targetPageId);
            if (targetPageId === "page-home") refreshHomeCarousels();
            return;
        }
    }

    // A sub-section/detail layer (Fill the Details, Confirm, Success, etc.) is about to
    // hide a top-level page — remember its scroll position before it disappears, so
    // coming back to it later can restore this exact spot instead of losing it.
    if (underlyingActiveId && TOP_LEVEL_PAGE_IDS.has(underlyingActiveId) && underlyingActiveId !== targetPageId) {
        topLevelScrollMemory[underlyingActiveId] = window.scrollY;
    }
    ...
    if (targetNode) {
        ...
        // Returning to a top-level page (Home/Dashboard/Game) from underneath a
        // sub-section/detail layer restores the exact scroll position the user had before
        // they left it. A direct primary switch between top-level pages themselves (tab
        // taps, or Home<->Dashboard<->Game in any order) instead always lands at the very
        // top. Either way the jump is instant — explicitly overriding the page's smooth
        // scroll-behavior — so there is never a visible auto-scrolling motion on screen.
        const isReturningFromSubsection = underlyingActiveId && !TOP_LEVEL_PAGE_IDS.has(underlyingActiveId);
        const restoreTop = (TOP_LEVEL_PAGE_IDS.has(targetPageId) && isReturningFromSubsection)
            ? (topLevelScrollMemory[targetPageId] || 0)
            : 0;
        window.scrollTo({ top: restoreTop, left: 0, behavior: "instant" });
    }
    updateBottomNavState(targetPageId);
    ...
    if (targetPageId === "page-home" || targetPageId === "page-game") {
...
/* =====================================================================================
   CAROUSEL REFRESH & BOTTOM-NAV STATE
===================================================================================== */
...
// Tracks which Dashboard sub-tab (Registered vs Winners) is currently showing, so the
// hardware/gesture back-guard below can step back to the default "Registered" tab first
// before leaving the Dashboard entirely, instead of always jumping straight Home.
let dashboardActiveTabId = "public-local-tab";

window.switchPublicTab = function(targetTabId) {
    dashboardActiveTabId = targetTabId;
...
/* PERF: same rAF-debounce pattern used by filterPublicDashboard() above — batches the
   filter pass onto the next animation frame so a fast typist or a held-down key produces
   one re-render instead of one per keystroke. Harmless at ~70 countries today, but keeps
   every search input in the app on one consistent, scalable pattern. */
let countryFilterFrameQueued = false;

function applyCountryModalFilter() {
    const searchBar = document.getElementById("country-search-bar");
    if (!searchBar) return;
    const term = searchBar.value.trim().toLowerCase();
    const filtered = countryDataList.filter(c => c.name.toLowerCase().startsWith(term) || c.code.startsWith(term));
    populateCountryList(filtered);
}

window.filterCountryModal = function() {
    if (countryFilterFrameQueued) return;
    countryFilterFrameQueued = true;
    requestAnimationFrame(() => {
        countryFilterFrameQueued = false;
        applyCountryModalFilter();
    });
}
...
window.processFormSubmission = async function() {
    ...
    // Minimum-length guards: the live keystroke sanitizers only restrict *character set*
    // (letters/spaces for name, digits for phone, forced "#" prefix for Game ID), so a
    // single-character value previously sailed through as "valid" and reached Firestore.
    if (nameVal.length < 2) {
        await customAlert("Invalid Name", "Please enter your full name (at least 2 characters).", "error");
        return;
    }
    if (phoneVal.length < 7) {
        await customAlert("Invalid Phone Number", "Please enter a valid WhatsApp number (at least 7 digits).", "error");
        return;
    }
    if (gameIdVal.replace(/^#/, "").length < 3) {
        await customAlert("Invalid Game ID", "Game ID must be at least 3 characters after the \"#\".", "error");
        return;
    }
...
/* SPLIT-DOCUMENT ENTRY WRITE
   Firestore read rules are all-or-nothing per document, and this site's public dashboard
   and duplicate-ID check both require public read access to entries. Keeping the phone
   number on that same document therefore made every entrant's WhatsApp number readable by
   anyone on the internet. The entry is now split across two documents sharing one id:

     submissions/{id}          name, gameId, status, timestamp   — public read
     submission_contacts/{id}  phoneNumber, age                  — admin read only

   Both are committed in a single writeBatch, so an entry can never land half-written.
   The Admin Panel and Lucky Draw (both authenticated) re-join them by id. */
/* =====================================================================================
...
window.commitSubmissionToFirebase = async function() {
    ...
    // Double-tap guard: without this, an impatient second tap on SUBMIT fires a second
    // batch before the first resolves and the entrant is registered twice.
    if (submissionInFlight) return;
    submissionInFlight = true;

    const submitBtn = document.querySelector("#page-confirm .btn-success-action");
    if (submitBtn) submitBtn.disabled = true;

    try {
        const { db, doc, collection, writeBatch } = window.FirebaseBridge;

        // Generate the shared id up front so both halves are addressable by the same key.
        const entryRef = doc(collection(db, "submissions"));
        const contactRef = doc(db, "submission_contacts", entryRef.id);
...
/* PERF: the search box fires oninput on every keystroke, and each call walked every row in
   the list doing two getAttribute() reads plus a class write. On a long entrant list that
   is a full style recalculation per character typed, which is precisely what makes typing
   feel sticky on a phone.

   Two fixes: the actual work is debounced onto the next animation frame (so holding a key
   down or a fast typist produces one pass, not one per character), and each row's class is
   only written when its visibility actually changes — an unchanged row costs nothing. */
/* =====================================================================================
...
function applyPublicDashboardFilter() {
    const searchBar = document.getElementById("public-search-bar");
    if (!searchBar) return;
    const queryTerm = searchBar.value.trim().toUpperCase();
    ...
    listRows.forEach(row => {
        ...
        const shouldShow = !queryTerm || idText.startsWith(queryTerm) || nameText.startsWith(queryTerm);
        const isHidden = row.classList.contains("hidden");
        if (shouldShow === isHidden) row.classList.toggle("hidden", !shouldShow);
    });
}
...
window.filterPublicDashboard = function() {
    ...
    requestAnimationFrame(() => {
        ...
        applyPublicDashboardFilter();
    });
}

/* =====================================================================================
   KEYBOARD-AWARE AUTO-SCROLL (Dashboard search + Country modal search)
   Both #public-search-bar (Dashboard) and #country-search-bar (Select Country modal)
   sit inside layouts that are deliberately body-scroll-locked (body.dashboard-active
   and body.modal-scroll-lock), so the normal "page scrolls up when the keyboard opens"
   behavior never happens on its own here — the soft keyboard just overlaps whatever was
   at the bottom of the screen. window.visualViewport reports the real, keyboard-aware
   viewport size/offset on every modern mobile browser, so it's used here to detect the
   keyboard and nudge only the relevant panel, without touching document/body scroll or
   any Firestore/data logic.
===================================================================================== */
function setupKeyboardAvoidance() {
    if (!window.visualViewport) return; // unsupported browser: no-op, layout is untouched
    const vv = window.visualViewport;
    const KEYBOARD_THRESHOLD_PX = 80; // ignore tiny viewport jitters (browser UI chrome, etc.)

    // ---- Dashboard: "SEARCH NAME OR ID" input --------------------------------------
    const dashSearchInput = document.getElementById("public-search-bar");
    const dashPanel = document.querySelector("#page-dashboard .max-width-dashboard");

    if (dashSearchInput && dashPanel) {
        const raiseDashboardPanel = () => {
            if (document.activeElement !== dashSearchInput) return;
            const keyboardHeight = Math.max(0, window.innerHeight - (vv.height + vv.offsetTop));
            if (keyboardHeight > KEYBOARD_THRESHOLD_PX) {
                // Shift the whole panel up just enough that the search bar sits near the
                // top of the visible (keyboard-shrunk) viewport. Capped so the shift never
                // over-scrolls past the panel's own content on shorter devices.
                const shiftPx = Math.min(keyboardHeight * 0.85, 230);
                dashPanel.style.transform = `translateY(-${shiftPx}px)`;
                dashPanel.classList.add("kb-shift-active");
            } else {
                dashPanel.style.transform = "";
                dashPanel.classList.remove("kb-shift-active");
            }
        };

        dashSearchInput.addEventListener("focus", () => {
            // Let the keyboard finish animating in before measuring, otherwise the very
            // first visualViewport reading can still reflect the pre-keyboard size.
            requestAnimationFrame(() => setTimeout(raiseDashboardPanel, 50));
            vv.addEventListener("resize", raiseDashboardPanel);
        });
        dashSearchInput.addEventListener("blur", () => {
            vv.removeEventListener("resize", raiseDashboardPanel);
            dashPanel.style.transform = "";
            dashPanel.classList.remove("kb-shift-active");
        });
    }

    // ---- "SELECT COUNTRY" modal search input ---------------------------------------
    const countrySearchInput = document.getElementById("country-search-bar");
    const countryOverlay = document.getElementById("country-modal-overlay");
    const countryModalBox = document.querySelector(".country-modal-box");

    if (countrySearchInput && countryOverlay && countryModalBox) {
        const raiseCountryModal = () => {
            if (document.activeElement !== countrySearchInput) return;
            const keyboardHeight = Math.max(0, window.innerHeight - (vv.height + vv.offsetTop));
            if (keyboardHeight > KEYBOARD_THRESHOLD_PX) {
                // Re-anchor the modal to the top of the shrunk viewport (instead of its
                // normal vertical-center) with a light offset, so the country list and the
                // custom-country-code section stay fully visible above the keyboard.
                countryOverlay.style.alignItems = "flex-start";
                countryModalBox.style.marginTop = "12px";
            } else {
                countryOverlay.style.alignItems = "";
                countryModalBox.style.marginTop = "";
            }
        };

        countrySearchInput.addEventListener("focus", () => {
            requestAnimationFrame(() => setTimeout(raiseCountryModal, 50));
            vv.addEventListener("resize", raiseCountryModal);
        });
        countrySearchInput.addEventListener("blur", () => {
            vv.removeEventListener("resize", raiseCountryModal);
            countryOverlay.style.alignItems = "";
            countryModalBox.style.marginTop = "";
        });
    }
...
/* =====================================================================================
   EVENT STATUS (OPEN / ENDED) UI STATE
===================================================================================== */
...
/* PERF: element lookups are cached once (getElementById on four nodes, once per second,
   forever, is pure waste) and each node is only written when its value actually changed.
   Assigning identical text to a node still dirties it for style recalculation, so on a
   typical tick this now touches exactly one element (seconds) instead of four. */
/* =====================================================================================
   REGISTRATION COUNTDOWN TIMER
===================================================================================== */

const regTimerNodes = { d: null, h: null, m: null, s: null, resolved: false };
function setRegTimerDisplay(d, h, m, s) {
    if (!regTimerNodes.resolved) {
        regTimerNodes.d = document.getElementById("rt-days");
        regTimerNodes.h = document.getElementById("rt-hours");
        regTimerNodes.m = document.getElementById("rt-minutes");
        regTimerNodes.s = document.getElementById("rt-seconds");
        regTimerNodes.resolved = true;
    }
    const pad = n => String(Math.max(0, n)).padStart(2, '0');
    const write = (el, val) => { if (el && el.textContent !== val) el.textContent = val; };
    write(regTimerNodes.d, pad(d));
    write(regTimerNodes.h, pad(h));
    write(regTimerNodes.m, pad(m));
    write(regTimerNodes.s, pad(s));
}
...
function startRegistrationCountdown(endTimestamp) {
    ...
    const tick = () => {
        ...
        if (diff <= 0) {
            setRegTimerDisplay(0, 0, 0, 0);
            const changed = registrationWindowExpired !== true;
            registrationWindowExpired = true;
            if (changed) updateEventUiState();
            if (registrationTimerInterval) { clearInterval(registrationTimerInterval); registrationTimerInterval = null; }
            return;
        }

        // PERF: updateEventUiState() touches five elements, toggles classes and can swap an
        // <img> src. Calling it unconditionally on every one-second tick meant the browser
        // re-ran style/layout once a second forever, for a result that is identical 99.9% of
        // the time. It now only runs on an actual open/expired transition.
        const changed = registrationWindowExpired !== false;
        registrationWindowExpired = false;
...
/* =====================================================================================
   HOME PAGE — PAST PRIZES PREVIEW & "SEE MORE" GRID
===================================================================================== */
    ...
    if (previewItems.length === 0) {
        ...
        previewItems.forEach(item => {
            ...
            const mediaMarkup = imgUrl
                ? `<img src="${escapeHtml(imgUrl)}" data-tz-orig-src="${escapeHtml(imgUrl)}" class="past-prize-thumb img-loading" alt="Past Prize" onload="this.classList.remove('img-loading')" onerror="handleTzImageError(this)">`
...
function buildSeeMoreGrid() {
    ...
    activePastPrizesData.forEach((item, index) => {
        ...
        const imgUrl = item.imageUrl || item.url || "";
        const mediaMarkup = imgUrl
            ? `<img src="${escapeHtml(imgUrl)}" data-tz-orig-src="${escapeHtml(imgUrl)}" class="see-more-img img-loading" alt="Past Prize" onload="this.classList.remove('img-loading')" onerror="handleTzImageError(this)" onclick="openImagePreview('${escapeHtml(imgUrl)}')">`
...
window.closeImagePreviewModal = function() {
    const overlay = document.getElementById("image-preview-overlay");
    if (!overlay.classList.contains("hidden")) unlockBodyScroll();
    overlay.classList.add("hidden");
}

// User-facing close (close-X button, backdrop tap): if the preview is the current back-
// stack layer, route it through a simulated back press so the stack stays in sync with
// what's on screen — otherwise a later hardware back press would silently do nothing.
window.dismissImagePreview = function() {
    if (currentNavLayer() === "image-preview") {
        goBackOneNavLayer();
    } else {
        closeImagePreviewModal();
    }
...
function applyLivePrizeConfig(imgUrl, captionStr) {
    ...
    if (imgUrl) {
        if (homeImg && homeImg.src !== imgUrl) {
            homeImg.classList.add("img-loading");
            // One automatic retry on a failed load before giving up and showing the
            // placeholder icon — recovers from transient network blips.
            attachImageRetryHandler(homeImg, imgUrl, () => {
                homeImg.classList.add("hidden");
                homeImg.classList.remove("img-loading");
...
function buildNoticeCarousel(items) {
    ...
    if (activeNoticeItems.length === 0) {
        ...
        activeNoticeItems.forEach(item => {
            ...
            const mediaMarkup = imgUrl
                ? `<img src="${escapeHtml(imgUrl)}" data-tz-orig-src="${escapeHtml(imgUrl)}" alt="Notice" class="prize-image img-loading" loading="eager" fetchpriority="high" decoding="async" onload="this.classList.remove('img-loading')" onerror="handleTzImageError(this)">`
                : `<div class="prize-fallback-placeholder">${IMAGE_PLACEHOLDER_SVG_MARKUP}</div>`;
...
/* =====================================================================================
   RENDER SCHEDULING (the main-thread smoothness fix)

   Firestore fires onSnapshot once per changed document during a burst — approving 40
   entries from the Admin Panel used to trigger 40 synchronous full-list innerHTML rebuilds
   back-to-back, each one re-parsing HTML and re-laying-out the whole scroller. That is the
   single worst stall on this page and it is exactly what "feels laggy" on a phone.

   Every render request is now coalesced into one rAF-aligned pass, so a burst of N updates
   costs one rebuild instead of N, and the rebuild lands in sync with the browser's paint
   cycle rather than blocking mid-frame. Renders for a tab that isn't on screen are deferred
   entirely until the user actually opens it.
===================================================================================== */
const pendingRenders = { dashboard: false, winners: false };
let renderFrameQueued = false;

function flushPendingRenders() {
    renderFrameQueued = false;
    if (pendingRenders.dashboard) { pendingRenders.dashboard = false; renderPublicDashboardNow(); }
    if (pendingRenders.winners) { pendingRenders.winners = false; renderPublicWinnerHistoryNow(); }
}

function scheduleRender(which) {
    pendingRenders[which] = true;
    if (renderFrameQueued) return;
    renderFrameQueued = true;
    requestAnimationFrame(flushPendingRenders);
}

// Public entry points keep their original names/signatures so every existing caller and
// the offline-fallback path are unchanged — they just queue instead of rendering inline.
function buildPublicDashboardUI() { scheduleRender("dashboard"); }
function buildPublicWinnerHistoryUI() { scheduleRender("winners"); }

function renderPublicDashboardNow() {
    ...
    // Built with .join() rather than repeated `html +=` string concatenation: on a list of
    // a few hundred entrants the incremental concat allocates a new string per row, which
    // is measurable GC churn on a low-end phone.
    const rows = new Array(verifiedRecords.length);
    verifiedRecords.forEach((player, idx) => {
        const safeName = escapeHtml(player.name);
        const safeId = escapeHtml(player.gameId);
        rows[idx] = `<div class="player-row-item" data-gameid="${safeId}" data-name="${safeName}">
                    <span>${idx + 1}. NAME: ${safeName}</span>
                    <span class="p-meta-id">ID: ${safeId}</span>
                </div>`;
    });
    scrollContainer.innerHTML = rows.join("");

    // Re-apply existing text filter state after rendering new sort. Called directly (not
    // through the debounced wrapper) so the freshly-rendered list is never briefly shown
    // unfiltered while a search term is active.
    applyPublicDashboardFilter();
}

function renderPublicWinnerHistoryNow() {
    ...
    const rows = new Array(activeFirestoreWinnerHistorySnapshot.length);
    activeFirestoreWinnerHistorySnapshot.forEach((player, idx) => {
        const dateStr = new Date(player.wonAt || Date.now()).toLocaleDateString();
        // The winner id is carried on a data attribute and read by a single delegated
        // listener below, instead of being interpolated into an inline onclick string.
        // Building executable JS out of record data is fragile (any quote in the value
        // breaks the handler) — a data attribute is inert and always correct.
        rows[idx] = `<div class="winner-card-item" data-winner-id="${escapeHtml(player.id)}">
...
// One delegated listener for the whole winners list, bound once. Previously every card
// carried its own inline onclick, so the browser compiled one function per row on every
// re-render; this binds a single handler for the lifetime of the page.
/* =====================================================================================
   GLOBAL CLICK DELEGATION (COPY / DOWNLOAD / IMAGE PREVIEW ACTIONS)
===================================================================================== */

document.addEventListener("click", (e) => {
    const card = e.target.closest(".winner-card-item[data-winner-id]");
    if (card) openWinnerModal(card.getAttribute("data-winner-id"));
});
...
window.openWinnerModal = function(winnerId) {
    ...
    if (prizeImgUrl && prizeImgUrl !== TRANSPARENT_PIXEL) {
        ...
        // Strict validation fallback: if the URL is broken / fails to load, retry once
        // after a short delay (recovers from transient network blips), and only swap to
        // the SVG icon if the retry also fails.
        attachImageRetryHandler(prizeImg, prizeImgUrl, showFallbackIcon);
    ...
    const winnerOverlay = document.getElementById("winner-modal-overlay");
    if (winnerOverlay.classList.contains("hidden")) lockBodyScroll();
    winnerOverlay.classList.remove("hidden");
}

window.closeWinnerModal = function() {
    const winnerOverlay = document.getElementById("winner-modal-overlay");
    if (!winnerOverlay.classList.contains("hidden")) unlockBodyScroll();
    winnerOverlay.classList.add("hidden");
}

// User-facing close (close-X button, backdrop tap): if the winner popup is the current
// back-stack layer, route it through a simulated back press so the stack stays in sync
// with what's on screen — otherwise a later hardware back press would silently do nothing.
window.dismissWinnerModal = function() {
    if (currentNavLayer() === "winner-modal") {
        goBackOneNavLayer();
    } else {
        closeWinnerModal();
    }
...
window.handleWinnerOverlayClick = function(e) {
    if (e.target.id === "winner-modal-overlay") dismissWinnerModal();
}
...
/* Hardened against non-string input. The previous version called .replace() directly on
   whatever it was handed, so any record where a field arrived as a number (an `age`, a
   numeric-looking gameId written by an older build) threw a TypeError mid-render and left
   the entire dashboard list blank. Coercing to string first makes it total. */
/* =====================================================================================
