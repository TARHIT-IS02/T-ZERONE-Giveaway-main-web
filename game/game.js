...
(function () {
  ...
  const ctx = canvas.getContext("2d", { alpha: true, desynchronized: true, powerPreference: "high-performance" });
  // Keep the render path deterministic and fast on high-refresh mobile displays.
  ...
  const backExitToastEl = document.getElementById("back-exit-toast");
  ...
  /* Pause screen's "BACK TO HOME PAGE" returns straight to the in-game Main
     Menu (screen-start) — plain menu navigation, no confirmation popup. */
  ...
  const DEFAULT_SETTINGS = { master: 100, homeMusic: 25, gameMusic: 40, sfx: 90, joySize: 120, joyMode: "dynamic" };
  ...
  const HERO_DEFS = [
    ...
    { key: "wizard", name: "WIZARD", desc: "Hurls explosive fireballs straight from his bare hands.", hp: 80, speed: 240, dmg: 13, range: 999, arc: 0, atkTime: 0.45, ranged: true, proj: { kind: "fire", speed: 420, color: "#FF9F0A" }, look: { skin: "#F2B279", hair: "#3B2A1A", cloth: "#1F4EA3", cloth2: "#3C7DE2", weapon: "hands", hood: true, orb: "#FF9F0A" } },
  ];
  function getHero() {
  ...
  function resize() {
    ...
    // 1.5x is a deliberate mobile quality/performance balance; desktop keeps up to 2x.
  ...
  const ENEMY_DEFS = {
    ...
    wizardE: { hp: 22, sp: 105, dmg: 12, range: 999, windup: 0.8, score: 190, gold: 12, r: 14, ranged: true, keepMin: 160, keepMax: 240, proj: { kind: "fire", speed: 210, color: "#3C7DE2", flame: true }, look: { skin: "#F2B279", hair: "#1B2A4A", cloth: "#1F4EA3", cloth2: "#3C7DE2", weapon: "hands", hood: true, orb: "#3C7DE2" } },
  ...
  const BOSS_DEFS = [
    ...
    { key: "wizardking", name: "DARK WIZARD KING", hp: 850, r: 42, speed: 82, dmg: 20, color: "#3C7DE2", look: { skin: "#C9A0E8", hair: "#12203E", cloth: "#1F4EA3", cloth2: "#3C7DE2", weapon: "hands", hood: true, orb: "#3C7DE2", scale: 1.9 } },
  ];
  ...
  function showWaveBanner(text, boss) {
    // Never create a gameplay notification unless the arena is actually active.
    if (state !== STATE.PLAYING) return;
  ...
  function drawWeapon(g, cx, cy, wa, look, r, st) {
    ...
    if (w === "fist" && look.twoHanded) {
      // Two-handed overhead smash — both fists rise together during windup
      // (driven by st.windupT) then drive down together into the ground on
      // impact (driven by st.slamT). An eased (smoothstep) curve replaces
      // the old linear raise so the swing accelerates/decelerates smoothly
      // instead of moving at a constant, robotic speed.
      const windT = st.windupT || 0;
      const slamT = st.slamT || 0;
      const ease = (t) => t * t * (3 - 2 * t); // smoothstep — natural accel/decel
      // raise: 0 = fists at sides, 1 = fists together overhead
      const raise = st.windup ? ease(windT) : (slamT > 0 ? ease(Math.max(0, 1 - slamT * 2.4)) : 0);
  ...
  function drawSkeletonUnit(g, x, cy, r, facing, look, st) {
    ...
    // no headwear — bare skull only, per design (cap intentionally removed)
  ...
  function drawCharacter(g, x, y, r, facing, look, st) {
    ...
    // detailed heater-shield while blocking. Two-handed fighters (e.g. the
    // Giant) already render BOTH hands as part of their weapon rig below, so
    // this generic off-hand is skipped for them — otherwise a third arm
    // would appear alongside the two-fisted weapon rig. ---
    const offSide = -Math.sign(Math.cos(facing) || 1);
    const offSway = -swing * 0.6;
    ...
    if (!look.twoHanded) {
      ...
      g.beginPath(); g.moveTo(offShoulderX, offShoulderY); g.lineTo(offHandX, offHandY); g.stroke();
      if (st.blocking) {
        drawShieldIcon(g, offHandX, offHandY, r * 0.42);
      } else {
    ...
    if (look.horns) {
      // Horns re-anchored to the sides of the helmet dome (rather than
      // floating above it) and curved forward/inward like a natural ram's
      // horn instead of a straight, spiky antenna, so they read as part of
      // the head rather than a stuck-on prop.
      const hornCol = "#6B2FBF", hornTip = "#9B6BFF";
      [-1, 1].forEach((side) => {
        const bx = side * r * 0.5, by = hy0 - r * 0.22;
        const mx = side * r * 0.66, my = hy0 - r * 0.52;
        const tx = side * r * 0.42, ty = hy0 - r * 0.74;
        g.fillStyle = OUTLINE;
        g.beginPath();
        g.moveTo(bx - side * r * 0.1, by + r * 0.06);
        g.quadraticCurveTo(mx + side * r * 0.09, my, tx + side * r * 0.05, ty);
        g.quadraticCurveTo(mx - side * r * 0.03, my + r * 0.1, bx + side * r * 0.12, by + r * 0.14);
        g.closePath(); g.fill();
        const hg = g.createLinearGradient(bx, by, tx, ty);
        hg.addColorStop(0, hornCol); hg.addColorStop(1, hornTip);
        g.fillStyle = hg;
        g.beginPath();
        g.moveTo(bx - side * r * 0.07, by + r * 0.05);
        g.quadraticCurveTo(mx + side * r * 0.06, my, tx + side * r * 0.03, ty);
        g.quadraticCurveTo(mx - side * r * 0.02, my + r * 0.08, bx + side * r * 0.09, by + r * 0.12);
        g.closePath(); g.fill();
        g.fillStyle = "rgba(255,255,255,0.4)";
        g.beginPath(); g.ellipse((bx + mx) / 2, (by + my) / 2, r * 0.03, r * 0.09, side * 0.9, 0, TAU); g.fill();
      });
    }
  ...
  function drawShieldIcon(g, x, y, size) {
    // Detailed heater-shield emblem — metal rim, riveted body, and a cross
    // motif — reused for the blocking pose, the shield pickup, and anywhere
    // else a recognizable "shield" needs to read clearly at a glance.
    g.save();
    g.translate(x, y);
    ...
    g.quadraticCurveTo(w * 0.5, h * 0.42, 0, h * 0.62);
    g.quadraticCurveTo(-w * 0.5, h * 0.42, -w * 0.5, h * 0.05);
    g.lineTo(-w * 0.5, -h * 0.38);
    g.closePath();
    g.fillStyle = "rgba(10,20,30,0.55)";
    g.save(); g.translate(size * 0.05, size * 0.06); g.fill(); g.restore();
    const bodyGrad = g.createLinearGradient(0, -h * 0.55, 0, h * 0.62);
    bodyGrad.addColorStop(0, "#BEE9FF"); bodyGrad.addColorStop(0.45, "#5AC8FA"); bodyGrad.addColorStop(1, "#2E7EB0");
    g.fillStyle = bodyGrad; g.fill();
    g.strokeStyle = "#0E3A54"; g.lineWidth = Math.max(1, size * 0.08); g.stroke();
    // inner rim highlight
    g.strokeStyle = "rgba(255,255,255,0.55)"; g.lineWidth = Math.max(0.8, size * 0.05);
    g.beginPath();
    g.moveTo(0, -h * 0.44);
    g.lineTo(w * 0.4, -h * 0.3);
    g.lineTo(w * 0.4, h * 0.02);
    g.quadraticCurveTo(w * 0.4, h * 0.32, 0, h * 0.5);
    g.stroke();
    // cross emblem
    g.fillStyle = "rgba(255,255,255,0.85)";
    g.fillRect(-w * 0.09, -h * 0.28, w * 0.18, h * 0.5);
    g.fillRect(-w * 0.28, -h * 0.08, w * 0.56, h * 0.16);
    // corner rivets
    g.fillStyle = "rgba(20,50,70,0.65)";
    [[-w * 0.32, -h * 0.2], [w * 0.32, -h * 0.2], [0, h * 0.32]].forEach(([rx, ry]) => {
      g.beginPath(); g.arc(rx, ry, size * 0.06, 0, TAU); g.fill();
    });
    g.restore();
  }
  function drawPickups(g) {
    for (const p of pickups) {
      ...
      if (p.kind === "gold") {
        ...
        drawShieldIcon(g, p.x, p.y + bob, 13);
      }
      g.globalAlpha = 1;
    }
  }
  function drawProjectiles(g) {
    for (const p of projs) {
      if (p.kind === "arrow") {
        // fully-formed arrow: wooden shaft, metal head with barbs, and
        // fletching feathers — replaces the old bare line so shots read as
        // real arrows in flight rather than abstract streaks.
        const a = Math.atan2(p.vy, p.vx);
        const len = 15;
        g.save();
        g.translate(p.x, p.y); g.rotate(a);
        // motion blur streak behind the shaft
        g.strokeStyle = "rgba(255,255,255,0.25)"; g.lineWidth = 1.4;
        g.beginPath(); g.moveTo(-len * 1.6, 0); g.lineTo(-len * 0.8, 0); g.stroke();
        // shaft
        const shaftGrad = g.createLinearGradient(-len * 0.6, 0, len * 0.35, 0);
        shaftGrad.addColorStop(0, "#7A4A22"); shaftGrad.addColorStop(1, "#B98A4A");
        g.strokeStyle = shaftGrad; g.lineWidth = 1.8; g.lineCap = "round";
        g.beginPath(); g.moveTo(-len * 0.6, 0); g.lineTo(len * 0.35, 0); g.stroke();
        // fletching (two small feather vanes near the nock)
        g.fillStyle = p.color;
        g.beginPath();
        g.moveTo(-len * 0.5, 0); g.lineTo(-len * 0.75, -2.6); g.lineTo(-len * 0.42, -0.6); g.closePath(); g.fill();
        g.beginPath();
        g.moveTo(-len * 0.5, 0); g.lineTo(-len * 0.75, 2.6); g.lineTo(-len * 0.42, 0.6); g.closePath(); g.fill();
        // arrowhead — diamond tip with barbs
        g.fillStyle = "rgba(30,30,34,0.65)";
        g.beginPath();
        g.moveTo(len * 0.62, 0);
        g.lineTo(len * 0.3, -2.4); g.lineTo(len * 0.14, -0.9);
        g.lineTo(len * 0.14, 0.9); g.lineTo(len * 0.3, 2.4);
        g.closePath(); g.fill();
        const headGrad = g.createLinearGradient(len * 0.1, -2, len * 0.6, 2);
        headGrad.addColorStop(0, "#E8ECF2"); headGrad.addColorStop(0.5, "#B7C1CD"); headGrad.addColorStop(1, "#767F8C");
        g.fillStyle = headGrad;
        g.beginPath();
        g.moveTo(len * 0.6, 0);
        g.lineTo(len * 0.28, -2.1); g.lineTo(len * 0.15, -0.8);
        g.lineTo(len * 0.15, 0.8); g.lineTo(len * 0.28, 2.1);
        g.closePath(); g.fill();
        g.strokeStyle = "rgba(255,255,255,0.5)"; g.lineWidth = 0.6;
        g.beginPath(); g.moveTo(len * 0.15, 0); g.lineTo(len * 0.55, 0); g.stroke();
        g.restore();
      } else if (p.kind === "fire") {
  ...
  // Browsers already schedule rAF at the panel refresh rate (60/90/120/144Hz).
  // We use frame-time based simulation, so gameplay speed never depends on FPS.
  function frame(now) {
    ...
    lastTime = now;
    // Prevent a tab switch / WebView stall from producing a physics teleport.
    if (dt > 0.05) dt = 0.05;
    if (dt < 0) dt = 0;

    // When hidden, do not burn CPU/GPU on an invisible canvas. The next visible
    // frame is clamped above, so the simulation resumes cleanly.
    ...
    if (state === STATE.PLAYING) {
      updateCooldownUI();
      // DOM text/layout work does not need to happen at 120Hz.
      if (now - lastHudRefresh >= 50) {
        updateHud();
        lastHudRefresh = now;
      }
    }
  ...
  function showScreen(name) {
    ...
    // Gameplay-only notifications must never remain visible over menus.
    // A screen transition is also the authoritative boundary for clearing
    // transient wave/floating notifications.
    if (name !== null) {
      hideGameplayNotifications();
    }

    if (name === "start" && resetHowtoAutoScroll) resetHowtoAutoScroll();
  }

  function hideGameplayNotifications() {
    if (waveBannerEl) {
      waveBannerEl.classList.remove("show", "boss-wave");
      waveBannerEl.textContent = "";
      waveBannerEl.setAttribute("aria-hidden", "true");
    }
    // WAVE CLEAR and other transient floating notifications are rendered on
    // the gameplay canvas, so discard them as soon as gameplay is left.
    floatingTexts = [];
  }
  ...
  function gameOver() {
    ...
    state = STATE.OVER;
    resetBackExitState(); // leaving active gameplay cancels any pending double-tap-to-exit
    hideGameplayNotifications();
  ...
  function setupCarousel(listEl, dotsEl, prevBtn, nextBtn) {
    ...
    // Calculate the card nearest the viewport center from scrollLeft/offsetLeft.
    // Unlike getBoundingClientRect(), this does not force a layout pass on every
    // scroll frame, which removes the main source of carousel hitching on mobile.
    function centerIndex() {
      const cs = allCards();
      if (!cs.length) return -1;

      const center = listEl.scrollLeft + listEl.clientWidth / 2;
      let best = 0;
    ...
    function paintCentered(force) {
      const cs = allCards();
      const idx = centerIndex();
      if (!force && idx === activeIndex) return idx;
      activeIndex = idx;

      // Only touch DOM classes when the focused card actually changes.
      for (let i = 0; i < cs.length; i++) {
        const centered = i === idx;
        if (cs[i].classList.contains("centered") !== centered) {
          cs[i].classList.toggle("centered", centered);
        }
      }
      ...
      if (dotsEl) {
        ...
        for (let i = 0; i < dots.length; i++) {
          const active = i === idx;
          if (dots[i].classList.contains("active") !== active) {
    ...
    function scrollToIndex(i, smooth) {
      ...
      // scrollTo is cheaper/more predictable than scrollIntoView because it
      // avoids ancestor alignment calculations.
      const target = Math.max(
        0,
        card.offsetLeft + card.offsetWidth / 2 - listEl.clientWidth / 2
      );
      listEl.scrollTo({
        left: target,
    ...
    listEl.addEventListener("scroll", () => {
      ...
      // Ensure the final snapped position is painted even on browsers that
      // throttle scroll events during a momentum gesture.
      clearTimeout(scrollEndTimer);
      scrollEndTimer = setTimeout(() => paintCentered(false), 80);
    }, { passive: true });
    ...
    if (nextBtn) {
      nextBtn.onclick = () => {
        const cs = allCards();
        const idx = centerIndex();
        if (idx >= cs.length - 1) return;
        sfx.ui();
        scrollToIndex(idx + 1, true);
      };
    }

    // Keep the focused card pixel-centered across viewport/orientation
    // changes. Debounced and non-smooth (behavior:auto) so a resize never
    // produces a stray animated scroll.
    let resizeTimer = null;
    window.addEventListener("resize", () => {
      clearTimeout(resizeTimer);
      resizeTimer = setTimeout(() => {
        const idx = activeIndex >= 0 ? activeIndex : centerIndex();
    ...
    return {
      refresh(focusKey) {
        ...
        requestAnimationFrame(() => {
          ...
          requestAnimationFrame(() => paintCentered(true));
        });
      },
      // Smoothly bring an already-rendered card to the front/center without
      // rebuilding the list or resetting scroll state. Used when a champion
      // is selected so the active card always becomes the front-most one.
      focus(focusKey, smooth) {
        const cs = allCards();
        if (!cs.length) return;
        const idx = cs.findIndex((c) => c.dataset.heroKey === focusKey);
        if (idx < 0) return;
  ...
  function renderHeroList(container) {
    ...
    HERO_DEFS.forEach((h) => {
      ...
      // The entire character card is a selection target. Ignore a click that
      // was actually a drag gesture so swiping never accidentally changes the
      // champion.
      let pointerStartX = 0, pointerStartY = 0, dragged = false;
  ...
  function updateHeroSelectionUI(container) {
    if (!container) return;
    const cards = container.children;
    for (let i = 0; i < cards.length; i++) {
      const card = cards[i];
      const selected = card.dataset.heroKey === selectedHero;
      card.classList.toggle("selected", selected);
      const btn = card.querySelector(".hull-select-btn");
      if (btn) {
  ...
  function selectHero(key) {
    if (!HERO_DEFS.some((h) => h.key === key)) return;
    if (selectedHero === key) {
      sfx.ui();
      return;
    }
    ...
    // Update the existing cards in place instead of rebuilding canvases and
    // resetting scroll state. This keeps both carousels visually stable and
    // removes selection-induced hitching.
    updateHeroSelectionUI(heroListEl);
    updateHeroSelectionUI(heroSelectListEl);

    // The newly selected champion becomes the front-most/centered card in
    // whichever carousel(s) are currently populated. Safe to call on a
    // carousel that hasn't been rendered yet (it just no-ops).
    if (heroCarousel) heroCarousel.focus(key, true);
    if (heroSelectCarousel) heroSelectCarousel.focus(key, true);
  }
  ...
  const joySizePct = document.getElementById("joy-size-pct");
  function refreshVolPercents() {
    ...
    if (joySizePct) joySizePct.textContent = settings.joySize + "%";
    ...
    if (joySizeInput) {
      const joyMin = Number(joySizeInput.min || 80), joyMax = Number(joySizeInput.max || 170);
      const joyPct = ((settings.joySize - joyMin) / (joyMax - joyMin)) * 100;
      joySizeInput.style.setProperty("--fill", Math.max(0, Math.min(100, joyPct)) + "%");
    }
  }
  ...
  function wireUI() {
    ...
    joySizeInput.oninput = (e) => { settings.joySize = +e.target.value; setSettings(settings); positionJoyEls(); refreshVolPercents(); if (state === STATE.PLAYING) showControls(true); };
  ...
  function wireResetHold() {
    ...
    resetHowtoAutoScroll = function() {
      ...
      viewport.classList.remove("howto-wrap-fade");
      setScrollTop(0);
    };
    ...
    function beginHold(e) {
      if (e) {
        e.preventDefault();
        // Keep receiving this exact pointer's events even after the finger
        // leaves the button. The hold ends only when contact is released or
        // the browser explicitly cancels the pointer.
    ...
    // Do NOT cancel on pointerleave: pointer capture deliberately keeps the
    // hold alive while the finger slides outside the button.
  ...
  function startHowtoAutoScroll() {
    ...
    let phase = "pausedTop", pauseUntil = performance.now() + PAUSE_MS, last = 0;
  ...
  /* ---------------- Home screen navigation ----------------
     Plain in-app "go to main menu" helper used by menu buttons. No custom
     hardware/browser back-button handling is installed anywhere: the device
     or browser back action is left to its normal, default behavior. */
  ...
  /* ---------------- Physical / hardware back-button handling ----------------
     Routes the device/browser back action (Android hardware back button,
     browser back gesture, etc.) through a single handler that mirrors the
     correct on-screen "back"/"cancel"/"resume" control for whatever is
     currently on top, instead of leaving the page uncontrolled:

       • Landing page                      -> exit game to the main website
       • Pre-battle champion-select popup   -> close popup, back to landing
       • Paused                             -> Resume (closes pause menu)
       • Barracks / Settings screens        -> same as their own back button
       • Active gameplay                    -> double-tap within 1.5s to exit
                                                to the game's home screen
       • Anything else (e.g. Game Over)     -> exit game to the main website

     Implemented with the standard pushState/popstate "guard" trick: a dummy
     history entry is (re)pushed after every handled back press so the next
     physical back press fires our popstate handler again instead of
     navigating the real browser history, EXCEPT when we are intentionally
     exiting the game, where real navigation is exactly what we want. */
  const BACK_EXIT_WINDOW_MS = 1500;
  let backExitArmed = false;
  let backExitTimer = null;

  function hideBackExitToast() {
    if (backExitToastEl) backExitToastEl.classList.remove("show");
  }
  function showBackExitToast() {
    if (backExitToastEl) backExitToastEl.classList.add("show");
  }
  ...
  function armBackExit() {
    ...
    backExitTimer = setTimeout(resetBackExitState, BACK_EXIT_WINDOW_MS);
  }

  function pushBackGuard() {
    try { history.pushState({ clashArenaBackGuard: true }, "", location.href); }
    catch (e) {}
  }

  function handleBackPress() {
    // 1) Pre-battle "select champion" popup open -> close it, land on the
    //    main landing page underneath (same as its CANCEL button).
    if (heroSelectModal && !heroSelectModal.classList.contains("hidden")) {
      sfx.ui();
      closeHeroSelect();
      pushBackGuard();
      return;
    }
    // 2) Paused -> Resume (same as the RESUME button).
    ...
    // 3) Barracks open -> same action as its on-screen back button.
    if (screenShop.classList.contains("active")) {
      const btn = document.getElementById("back-from-shop");
      if (btn) btn.click();
      pushBackGuard();
      return;
    }
    // 4) Settings open -> same action as its on-screen back button.
    if (screenSettings.classList.contains("active")) {
      const btn = document.getElementById("back-from-settings");
      if (btn) btn.click();
      pushBackGuard();
      return;
    }
    // 5) Active gameplay -> require a double-tap within 1.5s to exit.
    if (state === STATE.PLAYING) {
      if (backExitArmed) {
        resetBackExitState();
        sfx.ui();
    ...
    // 6) Landing page, Game Over screen, or any other resting state ->
    //    exit the game entirely back to the main website. This is a real
    //    navigation, so the guard is intentionally NOT re-pushed.
    goHome();
  }

  function wireBackButton() {
    window.addEventListener("popstate", handleBackPress);
    // Establish the initial guard entry once the app has booted so the
    // very first physical back press is caught by our handler above.
    pushBackGuard();
  }

  /* ---------------- Prevent image/element selection, long-press save, and
     right-click / long-press context menus anywhere in the game ---------------- */
  function wireSelectionGuards() {
